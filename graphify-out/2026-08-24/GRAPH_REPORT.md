...
## Corpus Check
- 148 files · ~540,766 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3103 nodes · 5406 edges · 223 communities (215 shown, 8 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 320 edges (avg confidence: 0.82)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c024d6d4`
...
### Community 4 - "free"
Cohesion: 0.11
Nodes (37): FILE, accept_if_exists(), build_curated_module(), build_from_toolchain_sources(), compiler_default_exe_path(), compiler_installed_runtime_hash(), compiler_publish_file(), compiler_runtime_source_hash() (+29 more)
...
Cohesion: 0.06
Nodes (33): LLVMErrorRef, check_llvm_version(), debug_dispose(), ensure_all_targets(), ensure_context(), ensure_target(), ir_arena_call(), ir_arena_hint_begin() (+25 more)
...
Cohesion: 0.10
Nodes (21): main(), LAYOUT 5's cost model ranks hot/cold cuts, and ranks them by cost rather     tha, The bootstrap scripts cache toolchain objects, and the key has to be content., INFERENCE 9's required check: an incremental result must equal a cold one., Nothing may take ordinal 0 in NodeKind or TypeKind.      A source check, because, The AIF memory model's tier derivation, one fixture per SPEC 4.2 clause.      As, SPEC 6.3's witness path, checked for shape rather than for prose.      The inter, M1.1's curated inlinable module: the set must be closed over exported symbols. (+13 more)
...
### Community 49 - "strcmp"
Cohesion: 0.16
Nodes (29): intern_value(), ir_alloc_cycle(), ir_alloc_rc(), ir_alloc_region(), ir_alloca(), ir_array_alloca(), ir_bitcast(), ir_call_end() (+21 more)
...
### Community 90 - "malloc"
Cohesion: 0.26
Nodes (19): free(), compile_ir_to_object(), compiler_bootstrap_executable(), compiler_build_executable(), compiler_prepare_output_path(), compiler_run_executable(), compiler_run_workload(), compiler_temp_obj_path() (+11 more)
...
Cohesion: 0.26
Nodes (12): now_ms(), run_boxed_aos(), run_boxed_split(), run_chunked_inline(), run_chunked_split(), run_inline_aos(), run_inline_split(), run_soa() (+4 more)
...
Cohesion: 0.28
Nodes (13): cyc_release(), list_release(), list_release_source(), list_set(), list_set_inline(), rc_alloc(), rc_attach_cold(), rc_cold_slot() (+5 more)
...
Cohesion: 0.22
Nodes (21): LLVMMetadataRef, diag_file_count(), diag_file_path(), di_basic(), di_cache(), di_cached(), di_enum_type(), di_field_type_name() (+13 more)
...
Cohesion: 0.14
Nodes (16): ir_get_struct_field_count(), ir_get_struct_field_type_at(), ir_is_struct_type_name(), ir_alloc_object(), ir_alloc_stack(), ir_const_str(), ir_copy_struct(), ir_free_cold() (+8 more)
...
Cohesion: 0.11
Nodes (19): emitted_layout_for(), LAYOUT 6's hot/cold split -- the release half, checked by running it.      A spl, The candidate `--layout` marks `emitted` for one type, as a label string.      R, `prismio bootstrap` builds a compiler, and the compiler it builds is right., LAYOUT 8's forced candidate -- `--force-layout=<Type>:<hot>`.      §8 selects a, `prismio run` with a forward-slash -o path.      Every other test goes through `, SPEC 5.2.1 -- the per-function bracketing summary reports the obligations,     a, `prismio run --jit`: the same program, without clang and without a link.      `r (+11 more)
...
Cohesion: 0.11
Nodes (27): compile_prismio_file(), _di_composite(), _di_located_lines(), _di_located_spans(), _di_nodes(), _di_scope_file(), _di_tuple(), _emitted_struct() (+19 more)
...
Cohesion: 0.20
Nodes (10): DeclEntry, add_binding(), decl_entry(), ir_decl_at(), ir_decl_count(), ir_index_decl(), ir_set_global_var_type(), ir_set_var_type() (+2 more)
...
Cohesion: 0.11
Nodes (22): LLVMBasicBlockRef, block_done(), block_for(), ir_br_numbered(), ir_call_begin(), ir_call_indirect_ptr(), ir_cond_br_numbered(), ir_declare_function_begin() (+14 more)
...
### Community 215 - "backend_fail"
Cohesion: 0.12
Nodes (17): backend_fail(), grow_table(), ir_append(), ir_append_line(), ir_br(), ir_call_arg(), ir_cond_br(), ir_const_cstring() (+9 more)
...
### Community 222 - "aif_layout_split_select"
Cohesion: 0.22
...
## Knowledge Gaps
- **979 isolated node(s):** `Darwin`, `Foundation`, `LLVMOpaqueContext`, `LLVMOpaqueModule`, `LLVMOpaqueType` (+974 more)
...
- **Why does `free()` connect `malloc` to `g6_bench.c`, `find_binding`, `free`, `ir_symbols.c`, `lang_runtime.c`, `cyc_hdr`, `diagnostics.c`, `The prompt for the next session`, `aif_place_arenas`, `g4.swift`, `run_aif_struct_field_test`, `str_split`, `add_binding`, `aif_verify_alloc`, `xrealloc`, `backend_fail`, `AIF — Measurement and Falsification Plan`, `aif_solve`, `aif_layout_split_select`, `nominal_find`?**
  _High betweenness centrality (0.120) - this node is a cross-community bridge._
- **Why does `di_enum_type()` connect `The prompt for the next session` to `malloc`, `ir_intern`, `llvm-api-backend.c`, `find_binding`?**

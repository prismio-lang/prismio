# Graph Report - prismio  (2026-08-26)

## Corpus Check
- 201 files · ~602,014 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3327 nodes · 5804 edges · 244 communities (232 shown, 12 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 346 edges (avg confidence: 0.82)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `cb000d59`
...
2. `malloc()` - 58 edges
3. `main()` - 46 edges
4. `Handoff — continuing the Prismio work` - 44 edges
...
## Communities (244 total, 12 thin omitted)
...
### Community 4 - "free"
Cohesion: 0.10
Nodes (49): FILE, accept_if_exists(), build_curated_module(), build_from_toolchain_sources(), build_trace_enabled(), build_trace_ms(), build_trace_stage(), compile_ir_to_object() (+41 more)
...
Cohesion: 0.15
Nodes (18): find_struct(), ir_get_enum_variant(), ir_get_enum_variant_count(), ir_get_enum_variant_name_at(), ir_get_enum_variant_value_at(), ir_get_struct_field_index(), ir_get_struct_field_name_at(), ir_get_struct_field_type() (+10 more)
...
Cohesion: 0.06
Nodes (36): LLVMErrorRef, check_llvm_version(), debug_dispose(), ensure_all_targets(), ensure_context(), ensure_target(), ir_append(), ir_append_line() (+28 more)
...
Cohesion: 0.09
Nodes (23): main(), LAYOUT 5's cost model ranks hot/cold cuts, and ranks them by cost rather     tha, The bootstrap scripts cache toolchain objects, and the key has to be content., Nothing may take ordinal 0 in NodeKind or TypeKind.      A source check, because, INFERENCE 9's required check: an incremental result must equal a cold one., SPEC 6.3's witness path, checked for shape rather than for prose.      The inter, The curated runtime merge is the default and really runs on this host.      The, The task handle has an owner, and only where AIF proved it may.      `prismio_ta (+15 more)
...
### Community 49 - "strcmp"
Cohesion: 0.15
Nodes (29): data_view_tbaa_tag(), global_named(), intern_value(), ir_bitcast(), ir_data_load_ptr(), ir_data_store_ptr(), ir_elem_ptr(), ir_extract_value() (+21 more)
...
Cohesion: 0.10
Nodes (15): DWORD, LPVOID, PrismioTask, append_module_name(), chan_free(), execute_command(), list_modules(), prismio_task_await() (+7 more)
...
Cohesion: 0.23
Nodes (13): now_ms(), run_boxed_aos(), run_boxed_split(), run_chunked_inline(), run_chunked_split(), run_inline_aos(), run_inline_split(), run_soa() (+5 more)
...
Cohesion: 0.08
Nodes (26): Current state, File roles, Five rules learned the hard way, Handoff — continuing the Prismio work, M2.1a-ii, second attempt: all three prerequisites built, all three reverted, M2 is closed, and four of its items are closed as blocked, Session of 2026-08-07 (second) — the corpus reaches zero, Session of 2026-08-23 (M2.0b) — the accumulator stops caring where its value came from (+18 more)
...
Cohesion: 0.20
Nodes (18): RtList, list_copy_elem(), list_inline_grow(), list_push(), list_push_inline(), list_release(), list_release_source(), list_set() (+10 more)
...
Cohesion: 0.09
Nodes (32): LLVMBasicBlockRef, backend_fail(), block_for(), const_from_text(), grow_table(), ir_alloca(), ir_array_alloca(), ir_br() (+24 more)
...
Cohesion: 0.08
Nodes (25): cleanup_files(), REQUIREMENTS 10, applied to the one module every build shares: the runtime., `prismio run` with a forward-slash -o path.      Every other test goes through `, The T0 path has to be checked in the IR, not only in the output: falling     bac, AIF Level 5, checked in the IR because neither half shows in a value.      The n, SPEC 8.4's E-VIEW, checked in the manifest and in the IR.      Both halves are l, M4.3's real-column, mutable round-trip, alias, and release gate., M4.4: generic clones choose storage only after T is concrete. (+17 more)

### Community 181 - "run_command"
Cohesion: 0.10
Nodes (21): emitted_layout_for(), LAYOUT 6's hot/cold split -- the release half, checked by running it.      A spl, The candidate `--layout` marks `emitted` for one type, as a label string.      R, LAYOUT 8's forced candidate -- `--force-layout=<Type>:<hot>`.      §8 selects a, SPEC 5.2.1 -- the per-function bracketing summary reports the obligations,     a, M4.1's discriminating representation, lifetime, and bounds gate., `prismio run --jit`: the same program, without clang and without a link.      `r, A packaged toolchain links the prebuilt runtime archive, and a foreign target do (+13 more)
...
Cohesion: 0.21
Nodes (10): compile_prismio_file(), expected_errors(), Substrings the diagnostics must contain, from `// expect-error:` lines.      Wit, `prismio bootstrap` builds a compiler, and the compiler it builds is right., SPEC 6.2 / 11 item 8 -- every record the manifest emits must be a record     the, run_bootstrap_command_test(), run_manifest_parseable_test(), run_negative_test() (+2 more)
...
Cohesion: 0.09
Nodes (30): arena_alloc(), arena_alloc_at(), arena_alloc_slot(), arena_current_slot(), data_view_add_column(), data_view_begin(), data_view_check_index(), data_view_finish() (+22 more)
...
Cohesion: 0.18
Nodes (11): Blocked, and on whom, Deliberately not in this list, How to work this, Read before starting, Read this before looking for something to do, Session prompt — compiler work, with no list, Start here — allocator evaluation is closed; prove replacement ownership next, Start here — the spawn argument is the open ownership question (+3 more)
...
Cohesion: 0.22
Nodes (10): DeclEntry, decl_entry(), hash_str(), ir_decl_at(), ir_decl_count(), ir_get_fn_return_type(), ir_index_decl(), ir_set_fn_return_type() (+2 more)
...
Cohesion: 0.13
Nodes (14): Concurrency + cold-compile handoff (2026-08-26, second session of the day), Current state, Fat String / native std.string handoff, M4.4 generic/container layout handoff (2026-08-25), M5.1 allocator handoff (2026-08-26), Native `strConcat`, O(1) String representation builtins, Performance evidence (+6 more)
...
Cohesion: 0.25
Nodes (8): 1 · Why the item existed, 2.1 The mechanism, and it is not a wash, 2 · Where Prismio stands, 3 · What the program found immediately, 4 · Defect 1 — the task handle had no owner. Fixed., 5 · Defect 2 — a callee-allocated spawn argument still leaks. Open., 6 · Gates, The concurrency axis
...
## Knowledge Gaps
- **1051 isolated node(s):** `Darwin`, `Foundation`, `LLVMOpaqueContext`, `LLVMOpaqueModule`, `LLVMOpaqueType` (+1046 more)
...
- **Why does `free()` connect `arena_chunk_new` to `g6_bench.c`, `ir_symbols.c`, `free`, `lang_runtime.c`, `arena_would_serve`, `cyc_hdr`, `llvm-api-backend.c`, `diagnostics.c`, `The prompt for the next session`, `g4.swift`, `aif_records`, `run_aif_struct_field_test`, `str_split`, `add_binding`, `list_new_cap`, `AIF — Measurement and Falsification Plan`, `aif_solve`, `nominal_find`, `aif_verify_alloc`, `add_binding`?**
...
- **Why does `ir_debug_begin()` connect `llvm-api-backend.c` to `ir_intern`, `arena_chunk_new`?**

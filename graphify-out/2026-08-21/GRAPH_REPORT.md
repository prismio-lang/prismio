...
## Corpus Check
- 131 files · ~451,230 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2828 nodes · 4937 edges · 214 communities (206 shown, 8 thin omitted)
...
## Communities (214 total, 8 thin omitted)
...
Cohesion: 0.06
Nodes (74): buildSystem(), countAlive(), countBeyond(), fade(), G1, integrate(), Particle, spawnParticle() (+66 more)
...
Cohesion: 0.11
Nodes (19): main(), LAYOUT 6's hot/cold split -- the release half, checked by running it.      A spl, The bootstrap scripts cache toolchain objects, and the key has to be content., INFERENCE 9's required check: an incremental result must equal a cold one., Nothing may take ordinal 0 in NodeKind or TypeKind.      A source check, because, Reclamation points, counted per function in the IR.      test_43 running clean a, SPEC 6.3's witness path, checked for shape rather than for prose.      The inter, The compiler and `aif/prototype/aif.py` must know the same builtins.      The di (+11 more)
...
### Community 49 - "strcmp"
Cohesion: 0.20
Nodes (23): global_named(), intern_value(), ir_alloca(), ir_array_alloca(), ir_bitcast(), ir_fneg(), ir_fptosi(), ir_fptoui() (+15 more)
...
Cohesion: 0.20
Nodes (23): now_ms(), run_boxed_aos(), run_boxed_split(), run_chunked_inline(), run_chunked_split(), run_inline_aos(), run_inline_split(), run_soa() (+15 more)
...
Cohesion: 0.20
Nodes (11): LLVMValueRef, NamedValue, apply_param_attrs(), debug_clear_location(), ir_debug_local(), ir_declare_function_end(), ir_function_body_start(), ir_load() (+3 more)
...
Cohesion: 0.19
Nodes (23): LLVMMetadataRef, diag_file_count(), diag_file_path(), ir_get_struct_field_count(), ir_get_struct_field_type_at(), di_basic(), di_cache(), di_cached() (+15 more)
...
Cohesion: 0.10
Nodes (26): backend_fail(), grow_table(), ir_br(), ir_call_arg(), ir_call_begin(), ir_call_end(), ir_cond_br(), ir_const_cstring() (+18 more)
...
Cohesion: 0.11
Nodes (19): cleanup_files(), REQUIREMENTS 10, applied to the one module every build shares: the runtime., `prismio run` with a forward-slash -o path.      Every other test goes through `, The T0 path has to be checked in the IR, not only in the output: falling     bac, AIF Level 5, checked in the IR because neither half shows in a value.      The n, A packaged toolchain links the prebuilt runtime archive, and a foreign target do, LAYOUT 7.2's field-order search, checked where it is decided: the IR.      The l, SPEC 7.1's zero-analysis mode, made falsifiable.      `src/aif.psm` used to carr (+11 more)
...
Cohesion: 0.29
Nodes (7): Five things now losing that are not accepted tradeoffs — the next list, `region` on g2 went from inert to 2.16×, which is the one large movement, Session of 2026-08-25 (final benchmark) — the corpus standing has not moved in seven sessions, and `region` is fixed, The architecture direction that follows, and the correction that re-ranks the roadmap, The claim under test is false, and the shape of the falsification matters, The runtime call boundary — found by asking why *hand-tuned* Prismio still loses, and it is the largest item here, Two methodology notes worth not re-deriving
...
## Knowledge Gaps
- **822 isolated node(s):** `Darwin`, `Foundation`, `1.1 Pipeline trace: `prismio build file.psm` → executable`, `1.2 How runtime libraries are included: embedded, external, or both?`, `1.3 Every file and function responsible` (+817 more)
...
- **Why does `free()` connect `strncpy` to `run_aif_struct_field_test`, `str_split`, `g6_bench.c`, `add_binding`, `free`, `ir_symbols.c`, `lang_runtime.c`, `Session of 2026-08-17 (second) — §8's forced candidate lands, and the IR differential turns out to have a concurrency hole`, `nominal_find`, `aif_verify_alloc`, `The prompt for the next session`, `g4.swift`, `ir_intern`, `arena_would_serve`, `cyc_hdr`, `diagnostics.c`, `aif_solve`?**

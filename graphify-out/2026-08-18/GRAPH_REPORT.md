# Graph Report - prismio  (2026-08-18)

## Corpus Check
- 124 files · ~369,373 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2537 nodes · 4561 edges · 177 communities (169 shown, 8 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 329 edges (avg confidence: 0.82)
...
### Community 4 - "free"
Cohesion: 0.10
Nodes (51): ArenaChunk, FILE, accept_if_exists(), build_from_toolchain_sources(), compile_ir_to_object(), compiler_build_executable(), compiler_default_exe_path(), compiler_installed_runtime_hash() (+43 more)

### Community 5 - "lang_runtime.c"
Cohesion: 0.04
Nodes (50): RtProfField, RtProfType, aif_live_hash(), aif_verify_alloc(), aif_verify_arm(), aif_verify_release(), arena_alloc(), arena_alloc_at() (+42 more)
...
Cohesion: 0.18
Nodes (15): CycHeader, cyc_alloc(), cyc_buffer(), cyc_collect(), cyc_collect_now(), cyc_final(), cyc_free_object(), cyc_hdr() (+7 more)
...
Cohesion: 0.08
Nodes (32): LLVMBasicBlockRef, block_done(), block_for(), check_llvm_version(), ensure_context(), ir_append(), ir_append_line(), ir_arena_call() (+24 more)
...
### Community 49 - "strcmp"
Cohesion: 0.23
Nodes (21): intern_value(), ir_array_alloca(), ir_bitcast(), ir_call_end(), ir_elem_ptr(), ir_fneg(), ir_fptosi(), ir_fptoui() (+13 more)
...
Cohesion: 0.29
Nodes (3): compare_names(), executable_directory(), prismio_executable_directory()

### Community 59 - "strncpy"
Cohesion: 0.12
Nodes (26): aif_layout_force(), diag_add_file(), diag_strdup(), ir_clear_local_var_types(), memcpy(), rt_alloc(), str_char_at(), str_clone() (+18 more)
...
Cohesion: 0.15
Nodes (16): strncpy(), ir_alloca(), ir_declare_function_begin(), ir_declare_function_param(), ir_function_begin(), ir_function_end(), ir_function_param(), ir_function_param_unique() (+8 more)
...
Cohesion: 0.13
Nodes (15): 1. The number the session exists to produce, 2. The regime question, settled and written into the spec, 3. What the reports say, and where the numbers live, 4. Two defects, one of them mine, and both now have a test that fails without the fix, 5. Why task 4 stopped, and the two things not to re-derive, 6. Fixtures built to discriminate, Current state, File roles (+7 more)
...
Cohesion: 0.15
Nodes (14): backend_fail(), const_from_text(), grow_table(), ir_br(), ir_call_arg(), ir_call_begin(), ir_cond_br(), ir_const_cstring() (+6 more)

### Community 175 - "struct_entry"
Cohesion: 0.17
Nodes (16): LLVMTypeRef, attach_cold(), attach_cold_rc(), global_named(), ir_alloc_cycle(), ir_alloc_object(), ir_alloc_rc(), ir_alloc_region() (+8 more)
...
## Knowledge Gaps
- **688 isolated node(s):** `Darwin`, `Foundation`, `1.1 Pipeline trace: `prismio build file.psm` → executable`, `1.2 How runtime libraries are included: embedded, external, or both?`, `1.3 Every file and function responsible` (+683 more)
...
- **Why does `strcmp()` connect `lang_runtime.c` to `free`, `g4.swift`, `nominal_find`, `aif_str`, `ir_intern`, `backend_fail`, `struct_entry`, `strcmp`, `LLVMValueRef`, `aif_intern`, `strncpy`, `6 · The tier manifest`?**
...
- **Are the 28 inferred relationships involving `strlen()` (e.g. with `aif_intern()` and `aif_layout_force()`) actually correct?**
  _`strlen()` has 28 INFERRED edges - model-reasoned connections that need verification._

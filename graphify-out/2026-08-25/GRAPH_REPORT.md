...
## Corpus Check
- 176 files · ~573,875 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3249 nodes · 5681 edges · 239 communities (229 shown, 10 thin omitted)
...
2. `malloc()` - 58 edges
3. `main()` - 43 edges
4. `Handoff — continuing the Prismio work` - 41 edges
5. `intern_value()` - 38 edges
6. `resolve_value()` - 37 edges
7. `main()` - 37 edges
8. `run_command()` - 37 edges
...
## Communities (239 total, 10 thin omitted)

### Community 0 - "malloc"
Cohesion: 0.04
Nodes (68): aif_call_edge(), aif_call_opaque(), aif_con_pin_region(), aif_elem_key(), aif_enum_new(), aif_extern_contract(), aif_extern_contract_set(), aif_field_access() (+60 more)
...
### Community 4 - "free"
Cohesion: 0.09
Nodes (53): FILE, accept_if_exists(), build_curated_module(), build_from_toolchain_sources(), compile_ir_to_object(), compiler_bootstrap_executable(), compiler_build_executable(), compiler_default_exe_path() (+45 more)
...
Cohesion: 0.07
Nodes (57): CallEdge, aif_arena_blockers(), aif_arena_high_water(), aif_arena_unsized_sites(), aif_bracket_callee(), aif_bracket_count(), aif_bracket_scope(), aif_bracket_served() (+49 more)
...
Cohesion: 0.06
Nodes (33): LLVMErrorRef, check_llvm_version(), debug_dispose(), ensure_all_targets(), ensure_context(), ensure_target(), ir_append(), ir_append_line() (+25 more)
...
Cohesion: 0.10
Nodes (21): main(), LAYOUT 6's hot/cold split -- the release half, checked by running it.      A spl, The bootstrap scripts cache toolchain objects, and the key has to be content., AIF Level 5, checked in the IR because neither half shows in a value.      The n, Nothing may take ordinal 0 in NodeKind or TypeKind.      A source check, because, INFERENCE 9's required check: an incremental result must equal a cold one., SPEC 7.3's verify mode, run over the fixtures that allocate structs.      The ha, SPEC 6.3's witness path, checked for shape rather than for prose.      The inter (+13 more)
...
### Community 49 - "strcmp"
Cohesion: 0.17
Nodes (27): data_view_tbaa_tag(), global_named(), intern_value(), ir_bitcast(), ir_data_load_ptr(), ir_data_store_ptr(), ir_extract_value(), ir_fneg() (+19 more)
...
Cohesion: 0.10
Nodes (17): DWORD, LPVOID, PrismioTask, append_module_name(), chan_free(), executable_directory(), execute_command(), list_modules() (+9 more)
...
Cohesion: 0.14
Nodes (34): Bits, Deriv, IntVec, aif_argv_push(), aif_solve(), aif_widen(), bits_any(), bits_clear() (+26 more)
...
Cohesion: 0.09
Nodes (23): Current state, File roles, Five rules learned the hard way, Handoff — continuing the Prismio work, M2.1a-ii, second attempt: all three prerequisites built, all three reverted, M2 is closed, and four of its items are closed as blocked, Session of 2026-08-07 (second) — the corpus reaches zero, Session of 2026-08-23 (M2.0b) — the accumulator stops caring where its value came from (+15 more)
...
Cohesion: 0.13
Nodes (33): aif_arena_at_node(), aif_cycle_at_node(), aif_elem_owner_at_node(), aif_field_is_counted(), aif_field_is_cyclic(), aif_field_release(), aif_frees_at_scope_node(), aif_owns_call_result_at_node() (+25 more)
...
Cohesion: 0.09
Nodes (31): NamedValue, backend_fail(), grow_table(), ir_alloca(), ir_array_alloca(), ir_br(), ir_call_begin(), ir_call_end() (+23 more)
...
Cohesion: 0.11
Nodes (27): LLVMTypeRef, ir_get_struct_field_count(), ir_get_struct_field_type_at(), ir_is_struct_type_name(), attach_cold(), attach_cold_rc(), const_from_text(), ir_alloc_cycle() (+19 more)
...
Cohesion: 0.09
Nodes (23): cleanup_files(), REQUIREMENTS 10, applied to the one module every build shares: the runtime., `prismio run` with a forward-slash -o path.      Every other test goes through `, The T0 path has to be checked in the IR, not only in the output: falling     bac, SPEC 8.4's E-VIEW, checked in the manifest and in the IR.      Both halves are l, M4.1's discriminating representation, lifetime, and bounds gate., LAYOUT 3 -- `workload`, and the three normative constraints that are     checkab, A packaged toolchain links the prebuilt runtime archive, and a foreign target do (+15 more)

### Community 181 - "run_command"
Cohesion: 0.10
Nodes (21): emitted_layout_for(), LAYOUT 5's cost model ranks hot/cold cuts, and ranks them by cost rather     tha, The candidate `--layout` marks `emitted` for one type, as a label string.      R, `prismio bootstrap` builds a compiler, and the compiler it builds is right., LAYOUT 8's forced candidate -- `--force-layout=<Type>:<hot>`.      §8 selects a, SPEC 5.2.1 -- the per-function bracketing summary reports the obligations,     a, M4.3's real-column, mutable round-trip, alias, and release gate., `prismio run --jit`: the same program, without clang and without a link.      `r (+13 more)
...
Cohesion: 0.40
Nodes (5): Correctness and closure gates, M4.3c — mutable DataView round trip, Mutable g1 layout gate, side by side with Rust, Standard-corpus regression gate, What changed
...
Cohesion: 0.15
Nodes (12): Current state, Fat String / native std.string handoff, M4.3c mutable DataView handoff (2026-08-25), Native `strConcat`, O(1) String representation builtins, Performance evidence, Recurring traps, Remaining `std.string` C calls removed (+4 more)
...
## Knowledge Gaps
- **1016 isolated node(s):** `Darwin`, `Foundation`, `LLVMOpaqueContext`, `LLVMOpaqueModule`, `LLVMOpaqueType` (+1011 more)
...
- **Why does `free()` connect `arena_chunk_new` to `run_aif_struct_field_test`, `str_split`, `g6_bench.c`, `add_binding`, `free`, `ir_symbols.c`, `lang_runtime.c`, `nominal_find`, `The prompt for the next session`, `g4.swift`, `xrealloc`, `arena_would_serve`, `AIF — Measurement and Falsification Plan`, `aif_verify_alloc`, `cyc_hdr`, `diagnostics.c`, `aif_records`, `aif_solve`?**

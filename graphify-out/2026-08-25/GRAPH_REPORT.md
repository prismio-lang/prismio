# Graph Report - prismio  (2026-08-24)

## Corpus Check
- 149 files · ~544,049 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3119 nodes · 5437 edges · 226 communities (218 shown, 8 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 320 edges (avg confidence: 0.82)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `56cfc59c`
...
2. `malloc()` - 54 edges
3. `main()` - 40 edges
4. `intern_value()` - 37 edges
5. `main()` - 37 edges
6. `bits_test()` - 36 edges
7. `Handoff — continuing the Prismio work` - 36 edges
8. `resolve_value()` - 35 edges
9. `run_command()` - 35 edges
...
## Communities (226 total, 8 thin omitted)

### Community 0 - "malloc"
Cohesion: 0.05
Nodes (63): aif_call_edge(), aif_call_opaque(), aif_con_pin_region(), aif_elem_key(), aif_enum_new(), aif_extern_contract(), aif_extern_contract_set(), aif_field_access() (+55 more)
...
### Community 4 - "free"
Cohesion: 0.11
Nodes (41): FILE, accept_if_exists(), build_curated_module(), build_from_toolchain_sources(), compiler_bootstrap_executable(), compiler_default_exe_path(), compiler_installed_runtime_hash(), compiler_prepare_output_path() (+33 more)
...
Cohesion: 0.20
Nodes (12): hash_str(), ir_get_enum_variant(), ir_get_enum_variant_count(), ir_get_enum_variant_name_at(), ir_get_enum_variant_value_at(), ir_get_fn_return_type(), ir_intern(), ir_is_reinit_target() (+4 more)
...
Cohesion: 0.13
Nodes (19): aif_arena_blockers(), aif_bracket_callee(), aif_bracket_count(), aif_bracket_scope(), aif_bracket_served(), aif_fn_calls_in_region(), aif_nearest_region_name(), aif_region_call_sites() (+11 more)

### Community 15 - "cyc_hdr"
Cohesion: 0.24
Nodes (12): CycHeader, cyc_alloc(), cyc_collect(), cyc_collect_now(), cyc_collect_white(), cyc_final(), cyc_free_object(), cyc_hdr() (+4 more)
...
Cohesion: 0.10
Nodes (21): main(), The bootstrap scripts cache toolchain objects, and the key has to be content., Nothing may take ordinal 0 in NodeKind or TypeKind.      A source check, because, INFERENCE 9's required check: an incremental result must equal a cold one., `prismio run --jit`: the same program, without clang and without a link.      `r, SPEC 7.1's zero-analysis mode, made falsifiable.      `src/aif.psm` used to carr, SPEC 6.3's witness path, checked for shape rather than for prose.      The inter, M1.1's curated inlinable module: the set must be closed over exported symbols. (+13 more)
...
### Community 49 - "strcmp"
Cohesion: 0.18
Nodes (26): global_named(), intern_value(), ir_array_alloca(), ir_bitcast(), ir_elem_ptr(), ir_extract_value(), ir_fneg(), ir_fptosi() (+18 more)
...
Cohesion: 0.10
Nodes (16): DWORD, LPVOID, PrismioTask, append_module_name(), chan_free(), executable_directory(), execute_command(), list_modules() (+8 more)
...
Cohesion: 0.05
Nodes (39): A constant shared across the seam has one spelling everywhere, A returned `String` must be freeable on every path, Allocations returned to Prismio go through `rt_base_alloc`, Before you commit, C code style, Comments, `extern fn` names are the ABI, Files and modules (+31 more)
...
### Community 90 - "malloc"
Cohesion: 0.25
Nodes (18): malloc(), compile_ir_to_object(), compiler_build_executable(), compiler_run_executable(), compiler_run_workload(), find_runtime_library(), link_against_runtime_library(), merge_curated_into_program() (+10 more)
...
Cohesion: 0.10
Nodes (50): Bits, CallEdge, Deriv, IntVec, aif_argv_push(), aif_bracketable_region_call_sites(), aif_fn_bracket_blockers(), aif_fn_bracket_hard_blockers() (+42 more)
...
Cohesion: 0.10
Nodes (35): Nominal, aif_compute_type_acyclic(), aif_field_is_counted(), aif_field_is_cyclic(), aif_field_release(), aif_layout_cand_bytes(), aif_layout_cand_field_hot(), aif_layout_field() (+27 more)
...
Cohesion: 0.11
Nodes (18): Current state, File roles, Five rules learned the hard way, Handoff — continuing the Prismio work, M2.1a-ii, second attempt: all three prerequisites built, all three reverted, M2 is closed, and four of its items are closed as blocked, Session of 2026-08-07 (second) — the corpus reaches zero, Session of 2026-08-23 (M2.0b) — the accumulator stops caring where its value came from (+10 more)
...
Cohesion: 0.15
Nodes (22): aif_arena_at_node(), aif_cycle_at_node(), aif_elem_owner_at_node(), aif_elem_type_at_node(), aif_frees_at_scope_node(), aif_layout_force(), aif_note_call_result(), aif_oom() (+14 more)
...
Cohesion: 0.08
Nodes (24): 0 · The one-paragraph answer, 10 · Reproducing, 1 · The full matrix, 2 · Prediction → session-3 measurement → now, per axis, 3 · The claim, stated the way the numbers support it, 4 · `region` on g2: session 3's sharpest negative result is fixed, 5.1 · The residual — the only design number, and it held, 5.2 · Executable size — still a large win, and it grew (+16 more)
...
Cohesion: 0.25
Nodes (14): cyc_buffer(), cyc_release(), list_release(), list_release_source(), list_set(), list_set_inline(), rc_alloc(), rc_attach_cold() (+6 more)
...
Cohesion: 0.25
Nodes (8): 1 · What this closes, 2 · The defect was documented, deliberate, and had stopped being true, 3 · The mechanism, 4 · The two things that cost the most to find, 5 · The fixture, and how it nearly measured nothing, 6 · Timing, 7 · What is left, measured, M2.0 — release on reassignment, and the M2 gate restated
...
Cohesion: 0.22
Nodes (23): LLVMMetadataRef, diag_file_count(), diag_file_path(), di_basic(), di_cache(), di_cached(), di_enum_type(), di_field_type_name() (+15 more)
...
Cohesion: 0.11
Nodes (23): LLVMTypeRef, NamedValue, backend_fail(), const_from_text(), ir_alloca(), ir_br(), ir_call_begin(), ir_call_end() (+15 more)
...
Cohesion: 0.15
Nodes (18): aif_arena_high_water(), aif_arena_range_first(), aif_arena_range_last(), aif_arena_unsized_sites(), aif_auto_arena_at_node(), aif_key_last_stmt(), aif_place_arenas(), aif_scope_served() (+10 more)
...
Cohesion: 0.10
Nodes (21): cleanup_files(), REQUIREMENTS 10, applied to the one module every build shares: the runtime., The T0 path has to be checked in the IR, not only in the output: falling     bac, AIF Level 5, checked in the IR because neither half shows in a value.      The n, SPEC 8.4's E-VIEW, checked in the manifest and in the IR.      Both halves are l, LAYOUT 3 -- `workload`, and the three normative constraints that are     checkab, A packaged toolchain links the prebuilt runtime archive, and a foreign target do, LAYOUT 7.2's field-order search, checked where it is decided: the IR.      The l (+13 more)

### Community 181 - "run_command"
Cohesion: 0.11
Nodes (19): emitted_layout_for(), LAYOUT 5's cost model ranks hot/cold cuts, and ranks them by cost rather     tha, LAYOUT 6's hot/cold split -- the release half, checked by running it.      A spl, The candidate `--layout` marks `emitted` for one type, as a label string.      R, `prismio bootstrap` builds a compiler, and the compiler it builds is right., LAYOUT 8's forced candidate -- `--force-layout=<Type>:<hot>`.      §8 selects a, `prismio run` with a forward-slash -o path.      Every other test goes through `, SPEC 5.2.1 -- the per-function bracketing summary reports the obligations,     a (+11 more)
...
Cohesion: 0.20
Nodes (12): compile_prismio_file(), _di_located_lines(), _di_located_spans(), _di_scope_file(), expected_errors(), Substrings the diagnostics must contain, from `// expect-error:` lines.      Wit, The DIFile a metadata node belongs to, following `scope:` upwards., Every line a DILocation names *in one source file*.      Pooling every file's lo (+4 more)
...
Cohesion: 0.12
Nodes (23): RtList, arena_alloc(), arena_alloc_at(), arena_alloc_slot(), cyc_retain(), int_to_str(), list_copy_elem(), list_inline_enabled() (+15 more)
...
Cohesion: 0.20
Nodes (10): Blocked, and on whom, Deliberately not in this list, How to work this, Read before starting, Read this before looking for something to do, Session prompt — compiler work, with no list, Start here — M2 is closed; M4 is next unless you want its blocked half, The standing gate (+2 more)
...
Cohesion: 0.50
Nodes (4): Appendix — M2's closing state, 2026-08-23, Delivered, Not delivered, and why, The honest headline
...
Cohesion: 0.10
Nodes (24): LLVMBasicBlockRef, block_done(), block_for(), grow_table(), ir_br_numbered(), ir_call_indirect_ptr(), ir_cond_br_numbered(), ir_declare_function_begin() (+16 more)

### Community 207 - "prismio_llvm.h"
Cohesion: 0.17
Nodes (11): LLVMOpaqueAttributeRef, LLVMOpaqueBasicBlock, LLVMOpaqueBuilder, LLVMOpaqueContext, LLVMOpaqueError, LLVMOpaqueModule, LLVMOpaquePassBuilderOptions, LLVMOpaqueTargetData (+3 more)
...
Cohesion: 0.23
Nodes (3): Prompt 2 — layout, Prompt 2 (original, for reference), The last-good generation
...
Cohesion: 0.17
Nodes (11): Current state, Fat String / native std.string handoff, Native `strConcat`, O(1) String length, Performance evidence, Recurring traps, Remaining `std.string` C calls removed, Sensible next work (+3 more)

### Community 214 - "Concepts"
Cohesion: 0.06
Nodes (32): Before trusting a single-program number, Borrow inference, Concepts, Cross-module inlining, Data views, Docs — part of the gate, not a follow-up, FBIP (functional but in-place), Free-list sharding (+24 more)

### Community 215 - "arena_chunk_new"
Cohesion: 0.22
Nodes (10): LLVMValueRef, apply_param_attrs(), coerce_for(), debug_clear_location(), ir_call_arg(), ir_curate_module(), ir_declare_function_end(), ir_delete_function_body() (+2 more)
...
Cohesion: 0.18
Nodes (11): find_binding(), ir_binding_owns_slot(), ir_binding_predates_loop(), ir_get_var_slot(), ir_get_var_type(), ir_has_var_type(), ir_mark_droppable(), ir_mark_mutable() (+3 more)

### Community 220 - "struct_entry"
Cohesion: 0.21
Nodes (12): attach_cold(), attach_cold_rc(), ir_alloc_cycle(), ir_alloc_object(), ir_alloc_rc(), ir_alloc_region(), ir_alloc_stack(), ir_struct_field_ptr() (+4 more)
...
Cohesion: 0.16
Nodes (14): calloc(), clock_gettime(), clock_mark(), count_calloc(), count_clock(), count_free(), count_malloc(), count_realloc() (+6 more)

### Community 223 - "find_struct"
Cohesion: 0.22
Nodes (11): find_struct(), ir_get_struct_field_count(), ir_get_struct_field_index(), ir_get_struct_field_name_at(), ir_get_struct_field_type(), ir_get_struct_field_type_at(), ir_is_struct_type_name(), ir_struct_is_flat() (+3 more)
...
Cohesion: 0.50
Nodes (4): Not this session, Prompt 2 (residual) — the hot/cold split, and only that, The one task, Verify, in this order
...
## Knowledge Gaps
- **988 isolated node(s):** `Darwin`, `Foundation`, `LLVMOpaqueContext`, `LLVMOpaqueModule`, `LLVMOpaqueType` (+983 more)
...
- **Why does `free()` connect `AIF — Measurement and Falsification Plan` to `g6_bench.c`, `ir_symbols.c`, `free`, `lang_runtime.c`, `cyc_hdr`, `diagnostics.c`, `The prompt for the next session`, `aif_place_arenas`, `5 · The accepted tradeoffs, reported anyway`, `g4.swift`, `run_aif_struct_field_test`, `str_split`, `add_binding`, `aif_verify_alloc`, `xrealloc`, `malloc`, `aif_solve`, `allocount.c`, `nominal_find`?**
  _High betweenness centrality (0.129) - this node is a cross-community bridge._
- **Why does `di_enum_type()` connect `The prompt for the next session` to `malloc`, `AIF — Measurement and Falsification Plan`, `ir_intern`, `llvm-api-backend.c`?**
...
- **Are the 63 inferred relationships involving `free()` (e.g. with `arena_reserve()` and `main()`) actually correct?**
  _`free()` has 63 INFERRED edges - model-reasoned connections that need verification._
- **Are the 51 inferred relationships involving `malloc()` (e.g. with `arena_reserve()` and `build_scene()`) actually correct?**
  _`malloc()` has 51 INFERRED edges - model-reasoned connections that need verification._

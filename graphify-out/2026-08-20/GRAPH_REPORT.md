...
## Corpus Check
- 124 files · ~394,332 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2632 nodes · 4718 edges · 195 communities (184 shown, 11 thin omitted)
...
### Community 4 - "free"
Cohesion: 0.11
Nodes (50): FILE, accept_if_exists(), build_from_toolchain_sources(), compile_ir_to_object(), compiler_bootstrap_executable(), compiler_build_executable(), compiler_default_exe_path(), compiler_installed_runtime_hash() (+42 more)

### Community 5 - "lang_runtime.c"
Cohesion: 0.04
Nodes (44): ArenaChunk, RtProfField, RtProfType, aif_live_hash(), aif_verify_alloc(), aif_verify_arm(), aif_verify_release(), arena_alloc() (+36 more)
...
Cohesion: 0.10
Nodes (30): aif_layout_force(), json_string_field(), diag_add_file(), diag_strdup(), ir_clear_local_var_types(), cyc_buffer(), cyc_walk_push(), memcpy() (+22 more)
...
## Knowledge Gaps
- **726 isolated node(s):** `Darwin`, `Foundation`, `1.1 Pipeline trace: `prismio build file.psm` → executable`, `1.2 How runtime libraries are included: embedded, external, or both?`, `1.3 Every file and function responsible` (+721 more)
...
- **Why does `Handoff — continuing the Prismio work` connect `Handoff — continuing the Prismio work` to `Session of 2026-08-08 (measurement) — the first cross-language numbers, and the optimiser was never on`, `Prompt 1 is done — what it unblocked`, `The cross-language suite — Prismio vs Rust vs Swift`, `str_split`, `aif_tier_of`, `Session of 2026-08-08 — the tree did not compile, and the T3 residue was never real`, `9 · The budget rule`, `Session of 2026-08-08 (measurement) — the first cross-language numbers, and the optimiser was never on`, `Session of 2026-08-09 (second) — views: the safety half landed, the speed half was somewhere else`, `Session of 2026-08-08 (measurement) — the first cross-language numbers, and the optimiser was never on`, `Session of 2026-08-19 (concurrency) — the `T` domain stops being vacuous, and T4a is emitted for the first time`, `Session of 2026-08-13 — `workload` lands; two of LAYOUT 6's dimensions are not blocked on what the brief said`, `arena_chunk_new`, `The cross-language suite — Prismio vs Rust vs Swift`, `Session of 2026-08-16 (second) — call-site placement lands; `region` stops being inert on g2`, `Session of 2026-08-16 (layout) — the cost model lands, and it could not have ranked the cut it exists for`?**

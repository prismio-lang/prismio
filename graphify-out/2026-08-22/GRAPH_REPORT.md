# Graph Report - prismio  (2026-08-22)

## Corpus Check
- 141 files · ~485,357 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2926 nodes · 5103 edges · 220 communities (212 shown, 8 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 322 edges (avg confidence: 0.82)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `84f42097`
...
3. `main()` - 38 edges
4. `main()` - 37 edges
5. `run_command()` - 34 edges
6. `bits_test()` - 33 edges
7. `intern_value()` - 33 edges
8. `resolve_value()` - 32 edges
9. `type_from_key()` - 30 edges
10. `Handoff — continuing the Prismio work` - 29 edges
...
## Communities (220 total, 8 thin omitted)

### Community 0 - "malloc"
Cohesion: 0.06
Nodes (44): CallEdge, aif_call_edge(), aif_call_opaque(), aif_con_pin_region(), aif_enum_new(), aif_extern_contract(), aif_extern_contract_set(), aif_fn_lookup() (+36 more)
...
### Community 4 - "free"
Cohesion: 0.11
Nodes (36): FILE, accept_if_exists(), build_curated_module(), build_from_toolchain_sources(), compiler_bootstrap_executable(), compiler_default_exe_path(), compiler_prepare_output_path(), compiler_temp_ir_path() (+28 more)
...
Cohesion: 0.20
Nodes (14): CycHeader, cyc_alloc(), cyc_buffer(), cyc_collect(), cyc_collect_now(), cyc_final(), cyc_free_object(), cyc_hdr() (+6 more)
...
Cohesion: 0.06
Nodes (37): LLVMErrorRef, apply_param_attrs(), check_llvm_version(), debug_clear_location(), debug_dispose(), ensure_all_targets(), ensure_context(), ensure_target() (+29 more)
...
Cohesion: 0.11
Nodes (19): main(), LAYOUT 6's hot/cold split -- the release half, checked by running it.      A spl, `prismio bootstrap` builds a compiler, and the compiler it builds is right., The bootstrap scripts cache toolchain objects, and the key has to be content., AIF Level 5, checked in the IR because neither half shows in a value.      The n, INFERENCE 9's required check: an incremental result must equal a cold one., Nothing may take ordinal 0 in NodeKind or TypeKind.      A source check, because, SPEC 6.3's witness path, checked for shape rather than for prose.      The inter (+11 more)
...
Cohesion: 0.02
Nodes (29): aif_check_pins(), aif_con_arg(), aif_con_bind(), aif_con_borrow(), aif_con_escape_caller(), aif_con_escape_global(), aif_con_live_in(), aif_con_no_stack() (+21 more)
...
### Community 49 - "strcmp"
Cohesion: 0.20
Nodes (23): global_named(), intern_value(), ir_array_alloca(), ir_bitcast(), ir_call_end(), ir_elem_ptr(), ir_fneg(), ir_fptosi() (+15 more)
...
Cohesion: 0.09
Nodes (18): DWORD, LPVOID, PrismioTask, append_module_name(), chan_free(), executable_directory(), execute_command(), get_directory() (+10 more)

### Community 59 - "strncpy"
Cohesion: 0.12
Nodes (21): calloc(), clock_gettime(), clock_mark(), count_calloc(), count_clock(), count_free(), count_malloc(), count_realloc() (+13 more)
...
Cohesion: 0.17
Nodes (13): aif_check_placement_pins(), aif_fn_name(), aif_fn_symbol(), aif_nominal_name(), aif_order_symbol(), aif_profile_source(), aif_region_name_at_site(), aif_scope_region() (+5 more)
...
Cohesion: 0.09
Nodes (32): aif_arena_at_node(), aif_arena_blockers(), aif_arena_high_water(), aif_arena_unsized_sites(), aif_bracket_callee(), aif_bracket_count(), aif_bracket_scope(), aif_bracket_served() (+24 more)
...
Cohesion: 0.12
Nodes (42): Bits, Deriv, IntVec, aif_argv_push(), aif_bracketable_region_call_sites(), aif_fn_bracket_blockers(), aif_solve(), aif_widen() (+34 more)
...
Cohesion: 0.15
Nodes (13): 0 · The diagnosis, and the one thing everybody had backwards, 1 · Close the runtime seam — the compilation architecture, 2 · Reuse analysis — the answer to 20–63× allocation, 3 · Regions: go non-lexical and polymorphic, 4 · Views and slices — one feature, two unlocks, 5 · The allocator — cheap, mechanical, and it also touches the RSS regression, 6 · The ranked plan, 7 · Measured dead ends — do not re-derive these (+5 more)
...
Cohesion: 0.22
Nodes (7): Not this session, Prompt 2 — layout, Prompt 2 (original, for reference), Prompt 2 (residual) — the hot/cold split, and only that, The last-good generation, The one task, Verify, in this order

### Community 130 - "find_binding"
Cohesion: 0.15
Nodes (19): LLVMTypeRef, LLVMValueRef, attach_cold(), attach_cold_rc(), const_from_text(), ir_alloc_cycle(), ir_alloc_object(), ir_alloc_rc() (+11 more)
...
Cohesion: 0.12
Nodes (16): 1. SPEC 8.4's E-VIEW landed, and it closes a real use-after-free, 2. The speed half: it was never the malloc, 3. The cross-language numbers, and the axis that did not exist, Current state, File roles, Five rules learned the hard way, Four things to carry forward, Handoff — continuing the Prismio work (+8 more)
...
Cohesion: 0.13
Nodes (30): aif_cycle_at_node(), aif_elem_owner_at_node(), aif_elem_type_at_node(), aif_field_is_counted(), aif_field_is_cyclic(), aif_field_release(), aif_frees_at_scope_node(), aif_owns_call_result_at_node() (+22 more)
...
Cohesion: 0.31
Nodes (11): list_push(), list_release(), list_set(), rc_alloc(), rc_attach_cold(), rc_cold_slot(), rc_release(), rc_release_atomic() (+3 more)
...
Cohesion: 0.17
Nodes (12): 2026-08-17 — tasks 1 and 2 both landed, in parallel, and were merged, 2026-08-19 (concurrency) — the `T` domain is live; the join analysis is the hole, 2026-08-21 (layering + targets) — worked; tasks 1–4 all landed, 2026-08-22 (finish `-g`) — worked; tasks 1–4 all landed, 2026-08-23 (packaged runtime) — candidate A, chosen over the optional task; worked, 2026-08-24 (the candidate list) — all five, in order; worked, 2026-08-25 — see `SESSION-PROMPT.md`, Prompts for the next sessions (+4 more)
...
Cohesion: 0.18
Nodes (24): LLVMMetadataRef, diag_file_count(), diag_file_path(), ir_get_struct_field_count(), ir_get_struct_field_name_at(), ir_get_struct_field_type_at(), di_basic(), di_cache() (+16 more)
...
Cohesion: 0.28
Nodes (15): compile_ir_to_object(), compiler_build_executable(), compiler_run_executable(), compiler_run_workload(), find_runtime_library(), inline_runtime_enabled(), link_against_runtime_library(), run_build_command() (+7 more)

### Community 175 - "struct_entry"
Cohesion: 0.06
Nodes (31): 0 · The answer, 10.1 · Emptying a function body without a `deleteBody`, 10.2 · Checked against the tool it replaces, 10.3 · It is also cheaper, 10.4 · The corpus, re-measured after the port, 10 · The merge moves in process, and the last blocker goes, 11.1 · Why neither option was the answer, 11.2 · What the corpus actually still called, and the false lead (+23 more)
...
Cohesion: 0.40
Nodes (5): Carry forward, Not this session, The prompt for the next session, The tasks, ranked, Verify, in this order
...
Cohesion: 0.11
Nodes (19): cleanup_files(), REQUIREMENTS 10, applied to the one module every build shares: the runtime., `prismio run` with a forward-slash -o path.      Every other test goes through `, SPEC 8.4's E-VIEW, checked in the manifest and in the IR.      Both halves are l, A packaged toolchain links the prebuilt runtime archive, and a foreign target do, LAYOUT 7.2's field-order search, checked where it is decided: the IR.      The l, Reclamation points, counted per function in the IR.      test_43 running clean a, SPEC 7.1's zero-analysis mode, made falsifiable.      `src/aif.psm` used to carr (+11 more)

### Community 181 - "run_command"
Cohesion: 0.12
Nodes (17): emitted_layout_for(), The candidate `--layout` marks `emitted` for one type, as a label string.      R, LAYOUT 8's forced candidate -- `--force-layout=<Type>:<hot>`.      §8 selects a, SPEC 5.2.1 -- the per-function bracketing summary reports the obligations,     a, The T0 path has to be checked in the IR, not only in the output: falling     bac, LAYOUT 3 -- `workload`, and the three normative constraints that are     checkab, `prismio run --jit`: the same program, without clang and without a link.      `r, The AIF memory model's tier derivation, one fixture per SPEC 4.2 clause.      As (+9 more)
...
Cohesion: 0.09
Nodes (31): LLVMBasicBlockRef, backend_fail(), block_for(), grow_table(), ir_alloca(), ir_br(), ir_br_numbered(), ir_call_arg() (+23 more)
...
Cohesion: 0.17
Nodes (13): arena_alloc(), arena_alloc_at(), arena_alloc_slot(), int_to_str(), list_push_grow(), rt_alloc(), str_clone(), str_concat() (+5 more)
...
Cohesion: 0.18
Nodes (11): NamedValue, block_done(), ir_call_indirect_ptr(), ir_debug_local(), ir_free_cold(), ir_load(), ir_ret(), ir_ret_void() (+3 more)
...
Cohesion: 0.25
Nodes (8): Blocked, and on whom, Deliberately not in this list, How to work this, Read before starting, Read this before looking for something to do, Session prompt — compiler work, with no list, The standing gate, wasm32 past IR
...
### Community 203 - "2026-08-20 (DWARF) — `-g` is in; the compiler itself still cannot be stepped through"
Cohesion: 0.33
...
Cohesion: 0.14
Nodes (21): find_struct(), hash_str(), ir_get_enum_variant(), ir_get_enum_variant_count(), ir_get_enum_variant_name_at(), ir_get_enum_variant_value_at(), ir_get_fn_return_type(), ir_get_struct_field_index() (+13 more)
...
### Community 207 - "scope_lca"
Cohesion: 0.27
Nodes (8): compile_prismio_file(), expected_errors(), Substrings the diagnostics must contain, from `// expect-error:` lines.      Wit, M1.1's curated inlinable module: the set must be closed over exported symbols., run_curated_closure_test(), run_negative_test(), run_program(), run_test()
...
### Community 211 - "2026-08-19 (generics) — monomorphisation is in; the stdlib floor is now gated on one missing primitive"
Cohesion: 0.40
...
### Community 212 - "The prompt for the next session"
Cohesion: 0.40
...
### Community 213 - "2026-08-17 (second) — LAYOUT §8's forced candidate landed; read this before writing another brief"
Cohesion: 0.24
Nodes (14): now_ms(), run_boxed_aos(), run_boxed_split(), run_chunked_inline(), run_chunked_split(), run_inline_aos(), run_inline_split(), run_soa() (+6 more)

### Community 214 - "Concepts"
Cohesion: 0.07
Nodes (28): Before trusting a single-program number, Borrow inference, Concepts, Cross-module inlining, Data views, Docs — part of the gate, not a follow-up, FBIP (functional but in-place), Free-list sharding (+20 more)

### Community 215 - "2026-08-17 (second) — LAYOUT §8's forced candidate landed; read this before writing another brief"
Cohesion: 0.50
Nodes (4): 2026-08-17 (second) — LAYOUT §8's forced candidate landed; read this before writing another brief, Next, ranked, Two things found, one fixed and one not, What landed here

### Community 216 - "milestone_bench.py"
Cohesion: 0.43
Nodes (6): build_prismio(), checksums(), main(), measure_ab(), Build one corpus program with one compiler. Returns the exe path., Interleaved A/B in ABBA order. Returns (per_run_a, per_run_b).      One warm pai
...
Cohesion: 0.25
Nodes (8): 5.1 · The residual — the only design number, and it held, 5.2 · Executable size — still a large win, and it grew, 5.3 · Peak RSS — this reversed, and it is on the next list, 5.4 · Toolchain size, 5.5 · Compile time — a win in the default configuration, a regression underneath it, 5.6 · Ecosystem, 5.7 · Allocator traffic, 5 · The accepted tradeoffs, reported anyway

### Community 219 - "add_binding"
Cohesion: 0.67
Nodes (3): add_binding(), ir_set_global_var_type(), ir_set_var_type()
...
## Knowledge Gaps
- **882 isolated node(s):** `Darwin`, `Foundation`, `1.1 Pipeline trace: `prismio build file.psm` → executable`, `1.2 How runtime libraries are included: embedded, external, or both?`, `1.3 Every file and function responsible` (+877 more)
...
- **Why does `free()` connect `strncpy` to `run_aif_struct_field_test`, `str_split`, `g6_bench.c`, `add_binding`, `free`, `diagnostics.c`, `ir_symbols.c`, `nominal_find`, `aif_verify_alloc`, `lang_runtime.c`, `The prompt for the next session`, `g4.swift`, `backend_fail`, `cyc_hdr`, `2026-08-17 (second) — LAYOUT §8's forced candidate landed; read this before writing another brief`, `aif_intern`, `add_binding`, `aif_solve`?**
  _High betweenness centrality (0.123) - this node is a cross-community bridge._
- **Why does `di_enum_type()` connect `The prompt for the next session` to `strncpy`, `find_struct`, `2026-08-17 (second) — LAYOUT §8's forced candidate landed; read this before writing another brief`, `llvm-api-backend.c`?**
  _High betweenness centrality (0.064) - this node is a cross-community bridge._
- **Why does `Handoff — continuing the Prismio work` connect `Handoff — continuing the Prismio work` to `Session of 2026-08-08 (measurement) — the first cross-language numbers, and the optimiser was never on`, `arena_chunk_new`, `Session of 2026-08-16 (second) — call-site placement lands; `region` stops being inert on g2`, `Session of 2026-08-16 (layout) — the cost model lands, and it could not have ranked the cut it exists for`, `Prompt 1 is done — what it unblocked`, `str_split`, `aif_tier_of`, `9 · The budget rule`, `2026-08-17 (compile time) — the frontend is linear now, and incrementality is not an AIF problem`, `The prompt for the next session`, `Session of 2026-08-19 (concurrency) — the `T` domain stops being vacuous, and T4a is emitted for the first time`, `run_cli_test`, `The cross-language suite — Prismio vs Rust vs Swift`, `run_aif_view_test`, `Session of 2026-08-08 (measurement) — the first cross-language numbers, and the optimiser was never on`, `Session of 2026-08-08 — the tree did not compile, and the T3 residue was never real`, `Session of 2026-08-22 (finishing `-g`) — four holes closed, and two bugs the tasks walked into`, `Session of 2026-08-24 (the candidate list) — all five taken in order, and the list is empty`, `Session of 2026-08-09 (second) — views: the safety half landed, the speed half was somewhere else`, `HANDOFF.md`, `Session of 2026-08-25 (final benchmark) — the corpus standing has not moved in seven sessions, and `region` is fixed`, `Session of 2026-08-17 (second) — §8's forced candidate lands, and the IR differential turns out to have a concurrency hole`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **Are the 62 inferred relationships involving `free()` (e.g. with `arena_reserve()` and `main()`) actually correct?**
  _`free()` has 62 INFERRED edges - model-reasoned connections that need verification._
- **Are the 58 inferred relationships involving `malloc()` (e.g. with `arena_reserve()` and `build_scene()`) actually correct?**
  _`malloc()` has 58 INFERRED edges - model-reasoned connections that need verification._

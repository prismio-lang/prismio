...
## Corpus Check
- 139 files · ~471,831 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2907 nodes · 5052 edges · 218 communities (210 shown, 8 thin omitted)
...
## Communities (218 total, 8 thin omitted)
...
### Community 4 - "free"
Cohesion: 0.10
Nodes (42): FILE, accept_if_exists(), build_curated_module(), build_from_toolchain_sources(), compile_ir_to_object(), compiler_bootstrap_executable(), compiler_default_exe_path(), compiler_installed_runtime_hash() (+34 more)
...
Cohesion: 0.10
Nodes (16): DWORD, LPVOID, PrismioTask, chan_free(), executable_directory(), execute_command(), get_directory(), prismio_executable_directory() (+8 more)
...
Cohesion: 0.31
Nodes (13): compiler_build_executable(), compiler_run_executable(), compiler_run_workload(), find_runtime_library(), link_against_runtime_library(), run_build_command(), run_command_path(), target_clang_flags() (+5 more)
...
Cohesion: 0.10
Nodes (21): cleanup_files(), REQUIREMENTS 10, applied to the one module every build shares: the runtime., `prismio run` with a forward-slash -o path.      Every other test goes through `, The T0 path has to be checked in the IR, not only in the output: falling     bac, AIF Level 5, checked in the IR because neither half shows in a value.      The n, SPEC 8.4's E-VIEW, checked in the manifest and in the IR.      Both halves are l, A packaged toolchain links the prebuilt runtime archive, and a foreign target do, LAYOUT 7.2's field-order search, checked where it is decided: the IR.      The l (+13 more)

### Community 181 - "run_command"
Cohesion: 0.12
Nodes (17): emitted_layout_for(), LAYOUT 6's hot/cold split -- the release half, checked by running it.      A spl, The candidate `--layout` marks `emitted` for one type, as a label string.      R, `prismio bootstrap` builds a compiler, and the compiler it builds is right., LAYOUT 8's forced candidate -- `--force-layout=<Type>:<hot>`.      §8 selects a, SPEC 5.2.1 -- the per-function bracketing summary reports the obligations,     a, LAYOUT 3 -- `workload`, and the three normative constraints that are     checkab, SPEC 6.2 / 11 item 8 -- every record the manifest emits must be a record     the (+9 more)
...
Cohesion: 0.50
Nodes (4): 2026-08-19 (concurrency) — the `T` domain is live; the join analysis is the hole, The tasks, ranked, Two things not to re-derive, What landed

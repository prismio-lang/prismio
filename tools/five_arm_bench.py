...
colder machine than the one that runs last. Here each round uses a Williams
balanced Latin-square order. Across a complete schedule every arm occupies
every position equally and immediately follows every other arm equally. For an
odd arm count the reversed square is appended, giving a 2N-round schedule.

It does not control binary layout, and nothing here can -- see the g5 note in
KNOWN_ISSUES.md. G5 therefore groups 32 renders into each clock interval, and a
per-function mnemonic diff is still what settles a single-program result before
any number below is interpreted.
...
The JSON keeps every raw sample, the exact build command, compiler hashes, and
the resulting executable hashes.  ``--tuned-ab`` adds an old-compiler build of
the hand-tuned Prismio source; ``--rebuild-aa`` adds independent tuned Prismio
and Rust rebuilds.  Those diagnostic arms make rebuild/layout noise visible
instead of folding it into an optimization claim.
"""
...
def arms_for(prog, old, new, rebuild_aa=False, tuned_ab=False):
    ...
    if os.path.exists(tuned):
        if tuned_ab:
            old_exe = os.path.join(OUT, f"{prog}_5arm_prismio_tuned_old")
            out.append(("prismio_tuned_old", "Prismio tuned (old)",
                        [old, "build", tuned, "-o", old_exe], old_exe))
        exe = os.path.join(OUT, f"{prog}_5arm_prismio_tuned")
        out.append(("prismio_tuned", "Prismio hand-tuned",
                    [new, "build", tuned, "-o", exe], exe))
        if rebuild_aa:
            aa_exe = os.path.join(OUT, f"{prog}_5arm_prismio_tuned_rebuild")
            out.append(("prismio_tuned_rebuild", "Prismio tuned rebuild A/A",
                        [new, "build", tuned, "-o", aa_exe], aa_exe))
    ...
    for key, label, stem in (("rust_idiomatic", "Rust idiomatic", "idiomatic"),
        ...
        out.append((key, label,
                    ["rustc", "-C", "opt-level=3", "--edition", "2021", src,
                     "-o", exe], exe))
        if rebuild_aa and key == "rust_tuned":
            aa_exe = os.path.join(OUT, f"{prog}_5arm_rust_tuned_rebuild")
            out.append(("rust_tuned_rebuild", "Rust tuned rebuild A/A",
                        ["rustc", "-C", "opt-level=3", "--edition", "2021", src,
                         "-o", aa_exe], aa_exe))
    return out
...
def balanced_orders(arms):
    """Williams design: balance both run position and first-order carryover."""
    n = len(arms)
    if n < 2:
        return [arms]
    first = []
    for pos in range(n):
        if pos == 0:
            ...
            first.append((pos + 1) // 2)
        else:
            first.append(n - pos // 2)
    rows = [[arms[(item + row) % n] for item in first]
            for row in range(n)]
    if n % 2:
...
def main():
    ...
    ap.add_argument(
        "--rebuild-aa", action="store_true",
        help="add independently rebuilt tuned Prismio and Rust A/A arms")
    ap.add_argument(
        "--tuned-ab", action="store_true",
        help="also build the tuned Prismio source with --old, so compiler "
             "changes can be measured on the exact expert workload")
    args = ap.parse_args()
    ...
    for prog in programs:
        arms = arms_for(prog, old, new, args.rebuild_aa, args.tuned_ab)
        ...
        if args.runs % len(orders):
            print(f"note: {args.runs} runs do not complete the "
                  f"{len(orders)}-round balance period", flush=True)
        ...
        results[prog] = {
            "checksums": list(distinct)[0],
            "schedule_period": len(orders),
            "arms": arm_json,
        }
    ...
    doc = {
        "meta": {
            ...
            "schedule": "Williams balanced Latin square (position and "
                        "first-order carryover balanced)",
            "rebuild_aa": args.rebuild_aa,
            "tuned_ab": args.tuned_ab,
            "old_compiler_sha256": sha256(old),
            "new_compiler_sha256": sha256(new),
